...
train_size = 2130 #int(len(data_normalized) * 0.7)
#train_set = data_normalized[:train_size]
train_set = data_normalized
#test_set = data_normalized
...
class AsymmetricLoss(nn.Module):
    ...
    def forward(self, y_pred, y_true):
        ...
        """  
        loss = torch.where(y_pred > y_true, 
                            self.low_weight * (y_pred - y_true) ** 2,  
                            200 * (y_true-y_pred) * (y_pred - y_true) ** 2)
        """
        return loss.mean()
...
model_skewed = train_model(loss_function = AsymmetricLoss(low_weight=1.0, medium_weight=4.0, high_weight=12.0), early_stop_loss=early_stop_loss_skewed, epochs=20)
...
real_res_print = real_res[input_seq_len:len(real_res)-42]
...
write_list_to_file(real_res_print.tolist(), file3)
